...
public class DynamicShardingManager {
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "maple_picture." + tableName) // 确保前缀合法
        ...
        Optional<ShadowRule> shardingRule = ruleMetaData.findSingleRule(ShadowRule.class);
        if (shardingRule.isPresent()) {
            ...
            log.error("未找到 ShardingSphere的分片规则配置,动态分表更新失败");
        }

    }

    /**
     * 获取Sharding ContextManager
     */
